...
HRESULT Quad::Initialize()
{
	...
	BufferMaker(vertices,index,4,6);
	return S_OK;
}
...
HRESULT Quad::BufferMaker(VERTEX* _v, int* _index, int _vNum, int _iNum)
{

	HRESULT hr;
	// 頂点データ用バッファの設定
	D3D11_BUFFER_DESC bd_vertex;
	bd_vertex.ByteWidth = sizeof(VERTEX)* _vNum;
...
void Quad::BufferSetter(XMMATRIX& worldMatrix)
{
	CONSTANT_BUFFER cb;
	cb.matWVP = XMMatrixTranspose(worldMatrix * Camera::GetViewMatrix() * Camera::GetProjectionMatrix());
